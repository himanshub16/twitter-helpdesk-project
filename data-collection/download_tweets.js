...
class TaskWorker {
    constructor(workToDo, client, is_parent = false) {
        ...
        this.workToDo = workToDo.filter(id => tweetsFound[id] == undefined)
        this.taskStatus = {}
        this.client = client
        this.left = 1
 ...
 //           this.parentWorker = new TaskWorker([], client, is_parent = true)
    ...
    async save(db) {
        if (this.results.length) {
            ...
            console.log('Saving from taskworker', res.insertedCount)
        }
    }
    ...
    async start(db) {
        ...
        for (let i = 0; i < n_pages; i++) {
            ...
            let response = await this.client.get('/statuses/lookup', {id: ids, include_entities: true, tweet_mode: 'extended'})
        ...
        //         let response = allResponses[i]
                ...
                console.log(`Page ${i+1} (${n_pages}). Found ${response.data.length} (${ids.length}) items. Pending: ${this.pendingReq - 1}. Results: ${this.results.length} (${this.workToDo.length}). Limit - left : ${this.left}`)

                let parentTweetIds = response.data.filter(tweet => tweet.in_reply_to_status_id_str).map(tweet => tweet.in_reply_to_status_id_str)
                if (parentTweetIds.length > 0) {
                    let parentWorker = new TaskWorker([], this.client)
                    parentWorker.workToDo = parentTweetIds
                    console.log(`Working for parent of page ${i}`)
                    await parentWorker.start(db)
                    await parentWorker.save(db)
                    parentWorker.results = []
                }
...
async function main() {
    ...
    let tasksFilename = null, skipPages = Number(process.argv[3]) || 0

    if (process.argv.length < 3) {
        console.error('Missing filename with tweet_ids to grab')
        console.error('Performing tasks already available in database.')
    } else {
        tasksFilename = process.argv[2]
    }

    const db = await connectToDB(env.db_url, env.db_name)

    let workToDo = await getWorkToBeDone(db, tasksFilename)
    workToDo = workToDo.slice(skipPages*100,)
